...
- **action**: 任意。定義登録時に Core-API が Registry へ照合し、未登録の ID はエラーになる。**省略時**は組み込みの `noop`（即時完了）と同等。**組み込み**として `delay5s`（約5秒待機後に完了、入出力なし）も利用できる。**`wait` を指定する状態では `action` と併記できない**。
