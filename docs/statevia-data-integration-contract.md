...
**Version 1.4（2026-05-20）**: immutable 定義版（`definitions` / `definition_versions`）と実行の `definition_version_id` 固定、publish の truth 先行順序を追記。詳細は [`core-api-interface.md`](./core-api-interface.md) §1.1。
...
- **実行固定:** `workflows.definition_version_id` — Start 時に解決した版を永続化。Engine には当該版の `compiled_json` を供給する。
...
- **現行の HTTP ステータス**: `POST /v1/workflows` は成功時 **`201 Created`**（ボディに `displayId` / `resourceId` 等）。`POST /v1/definitions` は成功時 **`201 Created`**（`DefinitionResponse`）。`PUT /v1/definitions/{id}` は成功時 **`200 OK`**（`DefinitionResponse`）。`POST /v1/workflows/{id}/cancel`・`POST /v1/workflows/{id}/events`・`POST /v1/workflows/{id}/nodes/{nodeId}/resume` は成功時 **`204 No Content`**。
- **任意ヘッダ**:
...
- **Graph Definition** の `edges[*].from` / `to` は **定義キャンバス上の `nodeId`**（状態名と一致しやすい）を指す。一方 **`GET /v1/workflows/{id}/graph`** の実行グラフでは **`from` / `to` が実行ノード ID**（`nodes[*].nodeId`）を指す（`docs/core-engine-execution-graph-spec.md`）。
...
- `patch.nodes`: `WorkflowViewMapper.MapGraphPatchNodes` の結果。各要素は **`executionNodeId`**（実行グラフの `nodeId`）をキーとするマップの値に相当し、**`stateName`**, **`status`**, **`attempt`**, **`workerId`**, **`waitKey`**, **`canceledByExecution`** 等を含み得る（詳細は `WorkflowViewDto` / `GraphPatchNodeDto` および `docs/core-api-interface.md` §3.5）。
