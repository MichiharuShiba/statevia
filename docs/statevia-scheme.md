...
**Version 1.6（2026-05-26）**: ExecutionSpace 命名統一（7a / 7c）。物理テーブル **`executions`**（旧 `workflows`）、**`execution_events`**（旧 `workflow_events`）、FK 列 **`execution_id`**、`display_ids.kind = execution`（旧 `workflow`）。HTTP `/v1/executions` は [`core-api-interface.md`](./core-api-interface.md) を参照。
