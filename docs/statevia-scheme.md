...
**Version 1.1（2026-05-05）**: `workflow_definitions` に `tenant_id`・`updated_at` を追記（EF マイグレーション実装に準拠）。作成時は `updated_at = created_at`、定義更新時は `updated_at` を更新する。
...
| tenant_id | varchar(64) | NOT NULL | テナント（`X-Tenant-Id` 省略時は `default`） |
| name | varchar(512) | NOT NULL | 定義名 |
| source_yaml | text | NOT NULL | 元の YAML |
| compiled_json | text | NOT NULL | コンパイル済み JSON |
| created_at | timestamptz | NOT NULL | 作成日時 |
| updated_at | timestamptz | NOT NULL | 最終更新日時（新規作成時は `created_at` と同一） |
