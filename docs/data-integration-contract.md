...
## すぐ“希薄さ”を解消する最短手
仕様を明文化したので、実装として最短で効くのはこれです：

1) `GET /graphs/:graphId`（構造の供給）
2) `GET /executions/:id/stream`（SSEで更新通知。まずは EXECUTION_UPDATED だけ）

これで UI は **構造×状態** を合成しつつ、更新も自然に追えるようになります。

---

次の返信で、あなたの現状Node+TS実装に合わせて
**SSEエンドポイントの最小実装**（PushはDB通知なしで、まずは簡易ポーリング型でもOK）と、`GET /graphs/:graphId` の最小実装（Hello固定でも可）をそのまま貼れます。
```
