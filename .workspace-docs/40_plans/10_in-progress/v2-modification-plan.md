...
Un（8.1）はすべて決定済み。以下 Cn のうち、方針決定で解消したものは打ち消し線で示す。残りは実装・仕様化で対応予定。**現在の決定内容を前提に洗い直した懸念**を列挙する。
...
| C2      | 一貫性       | **projection の更新タイミング**               | **U1 決定**に基づく。案 C では 1 バッチ = 1 トランザクションで INSERT + reducer + projection 更新。コマンド戻り値も 1 トランザクションで同様。実装時に一貫して適用する。                                                                                                           |
...
| C7      | 仕様         | **EventEnvelope と event_store の対応**       | **U1・U2 決定**後も、Engine がどのタイミングでどの Event を出すかは未仕様化。docs/core-events-spec の 24 種のイベントと event_store の対応を仕様化する必要がある。                                                                                                                 |
...
| C10     | API          | **一覧 API のページング**                     | GET /v1/definitions と GET /v1/workflows の一覧は実装済み（全件返却）。**ページング処理（limit / offset または cursor、hasMore、レスポンス形式）は未実装で、今後の課題**として残す。必要になった時点でパラメータ・仕様を決定する。                                                 |
| C11     | 永続化       | **コールバック失敗時のリトライ・再送**        | U1 案 C でバッチを 1 トランザクションで処理。バッチ失敗時のリトライで Engine が同一バッチを再送する場合、**event_id によるべき等・重複排除**の仕様が u1 の「失敗・リトライ」にオープンとして残る。実装時に確定する。                                                               |
...
| C13     | Engine       | **GetSnapshot と reducer 出力の関係**         | U7 で reducer は Engine に置き ExecutionState 相当を Engine が定義。**GetSnapshot はメモリ上のインスタンスから返すか、reducer 出力と統一するか**は U7 のオープン論点。API は GET /workflows/{id} を DB から返すため影響は限定的だが、デバッグ・一貫性で決めておくとよい。          |
| C14     | Phase 5      | **nodes 変換でカバーしきれない要素**          | U10 で判別方法は決定。nodes → states 変換で onError / timeout / controls 等は「変換レイヤーで解釈して擬似 state に落とす」方針。**変換で未対応の nodes 固有フィールド**があると Phase 5 で仕様ギャップが出る可能性。定義仕様（definition-spec.md）と変換仕様の突き合わせが未整備。 |
...
| 課題                            | 内容                                                                                                                                                                                                                                                                                                     |
| ------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **一覧 API のページング**       | GET /v1/definitions および GET /v1/workflows の**ページング処理**は未実装。limit / offset または cursor ベース、hasMore の有無、レスポンス形式などを必要に応じて仕様化・実装する。                                                                                                                       |
| **一覧 API のフィルタリング**   | GET /v1/definitions および GET /v1/workflows の**フィルタリング機能**（例: 定義名・ワークフロー status 等での絞り込み）は未実装。クエリパラメータや仕様は必要に応じて決定・実装する。                                                                                                                    |
| **Core-API のリファクタリング** | Core-API（C#）の**リファクタリング**は今後の課題とする。Controller / Repository / Service の責務分割、共通化、テスト容易性などを必要に応じて見直す。                                                                                                                                                     |
| **アーキテクチャの検討**        | API 層および Engine 連携の**アーキテクチャ**（レイヤー構成、DI、永続化の抽象化、エラーハンドリング等）は検討課題として残す。必要に応じて設計を見直す。                                                                                                                                                   |
| **UI の C# API レスポンス適合** | プロキシで /executions → /v1/workflows・/definitions → /v1/definitions のパス変換は実施済み。**UI 側で C# API のレスポンス形式（display_id / resource_id、status: Running 等）への適合**、および C# API に未実装の /stream・/state?atSeq=・/nodes/:id/resume の扱いは Phase 3.1 の残作業として実施する。 |
| **API レスポンスの camelCase シリアライズ** | Core-API（C#）の JSON レスポンスは今後 **camelCase** でシリアライズする予定。現状は Engine の ExportExecutionGraph 等が PascalCase のため、UI 型（WorkflowGraphNodeDTO 等）で camelCase / PascalCase 両対応している。camelCase に統一後は UI 側の PascalCase フォールバックは不要になる。 |
| **Idempotency Key による重複発行不可** | **TODO**: 同一の Idempotency-Key（例: ヘッダ `X-Idempotency-Key`）で同じコマンドが再送された場合、**重複してイベントを発行せず**、初回のレスポンスを返す（冪等保証）。Core-API でキーを永続化し、同一キー＋同一エンドポイント＋同一リクエストの再送時は 200/201 でキャッシュ済みレスポンスを返す。仕様・テーブル設計・実装は今後の課題とする。 |
