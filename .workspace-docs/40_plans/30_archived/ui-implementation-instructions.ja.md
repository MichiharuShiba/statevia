...
UIは Next Route Handler のプロキシ（`/api/core/*`）を介して Core-API（C#、api/）を呼び出す（CORS回避）。プロキシは executions → v1/workflows にマッピングする。

- GET  /api/core/executions/:id → Core-API GET /v1/workflows/:id
- POST /api/core/executions/:id/cancel → Core-API POST /v1/workflows/:id/cancel
- （ノード単位の resume は現行 API では POST /v1/workflows/:id/events でイベント発行）

レスポンス型は UI 側の型（services/ui/app/lib/types.ts）に従う。
...
- 状態の視覚強弱を実装（CANCELED 最強、WAITING 強調、RUNNING 控えめ）
...
- Core-API（api/）の仕様・挙動を変えること（UI側で吸収する）
