...
- コア仕様（core-engine-reducer-spec 等）の優先順位規則に従う。Codex への指示・レビューでも実装/テスト/ドキュメントで一貫させる。
...
- 例：`feat/cancel-wins-reducer` / `docs/core-api-interface`
