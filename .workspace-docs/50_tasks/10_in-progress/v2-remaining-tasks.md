...
- **完了**: `R1`～`R7`、`0.1`～`0.4`、`2.1`～`2.9`、`3.1`～`3.3`、`4.1`～`4.5`、`O1`～`O5`
- **未完了**: `O6`
- **見送り（今はしない）**: `O7`（認証機能追加時に着手）
- **棄却**: なし

補足:

- 実行チケットの主管理は `v2-ticket-backlog.md`（`STV-401`～`STV-411`）。
...
modification-plan 8.2（懸念）・8.3（今後の課題）から抽出。必要になった時点で仕様化・実装する想定。
...
| **O6** | 未完了 | 懸念の解消（仕様・実装） | P2 | - | C2/C7/C11/C13/C14 を必要に応じて仕様化・実装。 | `v2-ticket-backlog.md` STV-413〜STV-418、`.workspace-docs/30_specs/10_in-progress/o6-concerns_decomposition_spec.md` |
...
[その他 O]
  O1, O2, O3, O4, O5, O6 は優先度・必要に応じて実施
  O7（テナント ID 管理）は認証導入時に O7 とまとめて対応
```
...
8. **O1～O6** — 必要に応じてスプリントで取り込み
9. **O7**（テナント ID 管理）— **認証機能追加時に併せて対応**
...
| `.workspace-docs/50_tasks/10_in-progress/v2-ticket-backlog.md` | 実装チケット一覧（`STV-401`/`STV-402` 完了。未完は `STV-403`〜） |
