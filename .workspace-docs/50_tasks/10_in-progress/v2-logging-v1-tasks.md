...
- **完了**: `LOG-1`～`LOG-7`（`LOG-1`/`LOG-2` 受け入れ: `../20_done/v2-logging-stv403_backlog.md`, `../20_done/v2-logging-stv404_backlog.md`）
- **未完了**: なし
- **見送り（今はしない）**: なし
- **棄却**: なし
...
| **LOG-3** | 完了 | Warning ポリシー導入 | P2 | LOG-2 | 入力評価注意・遷移なし停止を Warning 出力 | `STV-405` |
| **LOG-4** | 完了 | `StateContext` に Logger を追加 | P2 | LOG-2 | `IState` 実装が `ctx.Logger` でログ出力可能 | `STV-406` |
| **LOG-5** | 完了 | ログフォーマット統一 | P2 | LOG-1, LOG-2 | `workflowId` / `stateName` / `traceId` のキー名統一 | `STV-407` |
| **LOG-6** | 完了 | 秘密情報マスキング | P2 | LOG-5 | input/output のログ方針を実装・文書化 | `STV-408`（ユーザー定義・外部テンプレートは `STV-412` で後続対応） |
| **LOG-7** | 完了 | テスト（最低限） | P2 | LOG-1〜LOG-6 | ログ呼び出しの単体テスト（重要経路）追加 | `STV-409` |
...
| 版 | 日付 | 内容 |
|----|------|------|
| 1.1.1 | 2026-04-10 | `LOG-6` 備考に `STV-412` の外部テンプレート化を反映（`v2-ticket-backlog.md` と整合）。 |
| 1.1.0 | 2026-04-09 | `LOG-5`/`LOG-6`/`LOG-7` を完了に更新。仕分けを `LOG-1`〜`LOG-7` 完了へ最新化。 |
| 1.0.3 | 2026-04-08 | `LOG-6` 備考にユーザー定義マスキングルールの後続チケット（`STV-412`）を追記。 |
| 1.0.2 | 2026-04-08 | `LOG-3`/`LOG-4` を完了に更新。仕分けを `LOG-1`〜`LOG-4` 完了へ最新化。 |
