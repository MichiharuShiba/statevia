...
- **完了**: `LOG-1`～`LOG-4`（受け入れ: `../20_done/v2-logging-stv403_backlog.md`, `../20_done/v2-logging-stv404_backlog.md`）
- **未完了**: `LOG-5` ～ `LOG-7`
- **見送り（今はしない）**: なし
- **棄却**: なし
...
| **LOG-6** | 未完了 | 秘密情報マスキング | P2 | LOG-5 | input/output のログ方針を実装・文書化 | `STV-408`（ユーザー定義ルールは `STV-412` で後続対応） |
