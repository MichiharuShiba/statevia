...
public sealed class DefinitionRepository : IDefinitionRepository
{
    ...
    public async Task<List<(WorkflowDefinitionRow Def, string? DisplayId)>> ListWithDisplayIdsAsync(string tenantId, CancellationToken ct)
    {
        ...
        var rows = await QueryDefinitionsWithDisplayIds(db, tenantId)
            ...
            .ToListAsync(ct)
            .ConfigureAwait(false);
        return rows.ConvertAll(x => (Def: x.Item1, DisplayId: x.Item2));
    }
        ...
        CancellationToken ct)
    {
        ...
        if (!string.IsNullOrWhiteSpace(nameContains))
            joinQuery = joinQuery.Where(x => x.Item1.Name.Contains(nameContains));
        ...
        List<(WorkflowDefinitionRow Def, string? DisplayId)> list = page.ConvertAll(x => (Def: x.Item1, DisplayId: x.Item2));
        return (total, list);
    }
    ...
    /// </summary>
    private static IQueryable<ValueTuple<WorkflowDefinitionRow, string?>> QueryDefinitionsWithDisplayIds(CoreDbContext db, string tenantId)
    {
