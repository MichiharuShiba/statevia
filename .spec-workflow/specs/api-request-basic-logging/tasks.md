...
- [ ] 1. TraceId 解決と `HttpContext.Items` キー
...
- [ ] 2. `RequestLogOptions` と `LogBodyRedactor`
...
- [ ] 3. レスポンスバッファリング `Stream` と `RequestLoggingMiddleware`
...
- [ ] 5. 単体テスト（TraceId / Redactor / Stream / ミドルウェア）
...
- [ ] 8. （任意）ルート解決後の ID enrich と `tracestate`（レビュー `comment_1775318191580_u2gvs3fdn`）
