...
- [-] **T3** — 共通部品への適用
  ...
  - _Requirements: Requirement 1, Requirement 2, Requirement 3_
  - _Definition of Done: 上記共通部品で表記ゆれが解消される（進捗: `layout.tsx` / `PageState.tsx` / `errors.ts` / `Toast.tsx` 適用済み）_
...
- [-] **T4** — 主要画面への展開
  ...
  - _Definition of Done: 主要画面で統一語彙が適用される（進捗: dashboard/definitions/workflows + execution 系主要コンポーネントに適用済み）_
