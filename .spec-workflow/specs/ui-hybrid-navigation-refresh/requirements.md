...
| No | アクター | きっかけ（ユースケース） | 期待される結果 |
| --- | --- | --- | --- |
| 1 | 利用者 | `/` にアクセスする | `redirect()` により `/dashboard` へ遷移する |
| 2 | 利用者 | `/dashboard` を開く | 直近 `WorkflowDetail` 10件が簡易表示される |
| 3 | 利用者 | 一覧項目を選択する | 対応する `WorkflowDetail` へ遷移する |
| 4 | 利用者 | 直近データが0件の状態で `/dashboard` を開く | 空状態メッセージと主要導線（定義一覧・ワークフロー一覧）が表示される |
