...
- [ ] **T1** — 言語仕様と辞書キーの確定
  ...
  - _Definition of Done: 要件と設計に同一ルールが明文化され、レビューで追跡できる_

- [ ] **T2** — i18n 基盤の追加（Locale / Dictionary）
  ...
  - _Requirements: Requirement 2, Requirement 3_
  - _Definition of Done: 型検査で辞書キー不足を検知でき、`getUiText` が利用可能になる_

- [ ] **T3** — Provider / Hook による文言参照統一
  ...
  - _Definition of Done: 主要画面で `uiText` を Provider 経由で参照できる_

- [ ] **T4** — 共通ヘッダーに言語切替 UI を追加
  ...
  - _Definition of Done: ヘッダー操作で言語が切り替わり、選択状態が視認できる_
...
- [ ] **T5** — 主要画面の文言参照を置換
  ...
  - _Definition of Done: 対象画面で言語混在なく切替表示される_
...
- [ ] **T6** — 日時ロケール対応の適用
  ...
  - _Definition of Done: 日本語/英語それぞれで自然な日時形式になる_
...
- [ ] **T7** — テストと品質確認
  ...
  - _Definition of Done: 変更範囲のテストと型検査が通過する_
