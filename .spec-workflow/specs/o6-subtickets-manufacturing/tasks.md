...
- [ ] 2. 専用 dedup リポジトリとステータス更新 API を実装する
...
- [ ] 3. Engine 公開インターフェースを clientEventId 対応で拡張する
...
- [ ] 4. Engine 内で AlreadyApplied 判定と結果モデルを実装する
...
- [ ] 5. WorkflowService を dedup 正本フローに置き換える
...
- [ ] 6. rollback 後の補正処理（projection再構築 + event_store insert-skip）を実装する
...
- [ ] 7. リトライポリシーとログ必須キーを実装する
  - 対象ファイル: `api/Statevia.Core.Api/Services/*`, `api/Statevia.Core.Api/Options/*`, `api/Statevia.Core.Api/Program.cs`, `api/Statevia.Core.Api.Tests/*`
