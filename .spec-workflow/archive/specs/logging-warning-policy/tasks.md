...
- [x] 1. WorkflowExecutionLogger への LogWarning メソッド追加
...
- [x] 2. Input 評価時の警告（Warning）検知と発火の実装
  - Files: `engine/Statevia.Core.Engine/Definition/StateInputEvaluator.cs`（`ApplyWithDiagnostics` と理由定数）、`engine/Statevia.Core.Engine/Engine/WorkflowEngine.cs`（`ApplyStateInput` から `_workflowLog.LogWarningInputEvaluation` を呼ぶ）
  - `ApplyStateInput` における評価時に、フォールバックや問題が起きていないかフックする。
  - 問題があれば `_workflowLog.LogWarningInputEvaluation` を呼び出す。
...
- [x] 3. 遷移なし停止時の警告（Warning）検知と発火の実装
  - File: `engine/Statevia.Core.Engine/Engine/WorkflowEngine.cs`
  - `ProcessFact` および `RunJoinStateAsync` 内で FSM 評価結果（`transition`）を検証する。
  - `HasTransition == false` であり、かつ `End == false` の場合は `_workflowLog.LogWarningNoTransition` を呼ぶ（Join 後に次状態がない場合の `else` も含む）。
